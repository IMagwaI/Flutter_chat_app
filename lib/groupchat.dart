...
class ChatScreen extends StatefulWidget {
  ...
  @override
  State createState() =>
      ChatScreenState(groupChatId: convId, users: users, title: title);
}
  ...
  editDB() async {
    ...
    FirebaseFirestore.instance.runTransaction((transaction) async {
      transaction.set(
        ...
        {'id': groupChatId, 'users': usersIDs, 'group': 1, 'title': title},
      );
    });

    for (int k = 0; k < usersIDs.length; k++) {
        try{
          print(k);
          var querySnapshot = await FirebaseFirestore.instance
              ...
              .doc(usersIDs[k])
              .get();
          List<String> conversations = List.from(querySnapshot["conversations"]);
          var i = true;
        ...
        return Container(
            child: StreamBuilder<QuerySnapshot>(
                stream: FirebaseFirestore.instance
                    .collection('users')
                    .where('id', isEqualTo: document.get('idFrom'))
                    .snapshots(),
                builder: (context, snapshot) {
                  var name ="";
                  var photo ="";
                  try {
                    name = snapshot.data!.docs[0].get('nickname');
                    photo = snapshot.data!.docs[0].get('photoUrl');
                  }catch(e){print(e);}
                  return Container(
                    child: Column(
                      children: <Widget>[
